  ...
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const read of readsArray) {
      const readsStartDate =
        read.startDate && !isNaN(new Date(read.startDate).getTime())
          ? new Date(read.startDate)
          : null;
      const readsEndDate =
        read.endDate && !isNaN(new Date(read.endDate).getTime())
          ? new Date(read.endDate)
          : null;
      const certificateIssuanceStartDate =
        read.certificateIssuanceStartDate &&
        !isNaN(new Date(read.certificateIssuanceStartDate).getTime())
          ? new Date(read.certificateIssuanceStartDate)
          : null;
      const certificateIssuanceEndDate =
        read.certificateIssuanceEndDate &&
        !isNaN(new Date(read.certificateIssuanceEndDate).getTime())
          ? new Date(read.certificateIssuanceEndDate)
          : null;
