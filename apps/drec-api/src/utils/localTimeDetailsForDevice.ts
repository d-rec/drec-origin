...
export const  getLocalTimeZoneFromDevice=(localTime, device)=> {
  ...
  else if(device.timezone)
  {
   ...
   return countryCodeFound.timezones[0];
  }
}
...
export const getOffsetFromTimeZoneName=(givenTimeZone)=>{
 ...
 for(let i=0;i<countryCodesList.length;i++)
 {
  ...
  {
    console.log('elementTimeZone::::'+elementTimeZone[j]);
    if(elementTimeZone[j].name===givenTimeZone)
    {
      ...
      console.log("TIMEZONE NOT MATCHING:::::::::::"+elementTimeZone[j].name);
      continue;
    }
  }
 }
...
const hours = Math.floor(givenOffSet/ 60);
