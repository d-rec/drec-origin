...
export class OnChainWatcher implements OnModuleInit {
  ...
  public async onModuleInit(): Promise<void> {
    ...
    this.provider.pollingInterval =
      getConfiguration().BLOCKCHAIN_POLLING_INTERVAL ||
      this.provider.pollingInterval;

    this.provider.on(
      this.registry.filters.IssuanceSingle(null, null, null),
      (event: providers.Log) =>
        this.processEvent(BlockchainEventType.IssuanceSingle, event),
    );

    this.provider.on(
      this.registry.filters.IssuanceBatch(null, null, null),
      (event: providers.Log) =>
        this.processEvent(BlockchainEventType.IssuanceBatch, event),
    );

    this.provider.on(
      this.registry.filters.TransferSingle(null, null, null, null, null),
      (event: providers.Log) =>
        this.processEvent(BlockchainEventType.TransferSingle, event),
    );

    this.provider.on(
      this.registry.filters.TransferBatch(null, null, null, null, null),
      (event: providers.Log) =>
        this.processEvent(BlockchainEventType.TransferBatch, event),
    );
    ...
    this.provider.on(
      ...
      (event: providers.Log) =>
        this.processEvent(BlockchainEventType.ClaimSingle, event),
    );

    this.provider.on(
      this.registry.filters.ClaimBatch(null, null, null, null, null, null),
      (event: providers.Log) =>
        this.processEvent(BlockchainEventType.ClaimBatch, event),
    );

    this.provider.on(
      this.registry.filters.ClaimBatchMultiple(
        null,
        null,
        null,
        null,
  ...
  async processEvent(
    ...
    rawEvent: providers.Log,
  ): Promise<void> {
    this.logger.debug(
      `Processing event ${eventType}: ${JSON.stringify(rawEvent)}`,
    );
    ...
    switch (eventType) {
      case BlockchainEventType.IssuanceSingle:
        this.logEvent(BlockchainEventType.IssuanceSingle, [
          event._id.toNumber(),
        ]);
    ...
    await this.transactionPollService.saveTransactionAsProcessed(
      event.transactionHash,
    );
  }
  ...
  private logEvent(type: BlockchainEventType, ids: number[]) {
    this.logger.log(
      `Detected a new event: ${type} on Certificate ${JSON.stringify(ids)}`,
    );
  }
