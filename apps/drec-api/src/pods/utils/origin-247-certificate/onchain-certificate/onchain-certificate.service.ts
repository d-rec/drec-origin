...
export class OnChainCertificateService<T = null> {
  ...
  public async claimWithTxHash(
    command: IClaimCommand,
  ...
  public async transferWithTxHash(
  ...
  public async batchIssueWithTxHash(
    ...
    this.logger.debug(
      `Adding ${originalCertificates.length} certificates to issue to queue`,
    );
  ...
  public async batchIssue(
    ...
    const { certificateIds } =
      await this.batchIssueWithTxHash(originalCertificates);

    return certificateIds;
  }

  public async batchClaimWithTxHash(
    command: IClaimCommand[],
  ...
  private mapCertificate(certificate: ICertificate<any>): ICertificate<T> {
    return {
      ...
      metadata:
        certificate.metadata !== '' ? JSON.parse(certificate.metadata) : null,
    };
  }
