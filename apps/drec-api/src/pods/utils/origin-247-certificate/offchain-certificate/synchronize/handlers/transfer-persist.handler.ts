...
export class TransferPersistHandler {
  ...
  public async handleBatch(
    ...
    for (const eventBlock of eventsBlocks) {
      const result = await this.synchronizeBatchBlock(
        eventBlock,
        blockchainIdMap,
      );
  ...
  private async synchronizeBatchBlock(
    ...
    try {
      const { transactionHash } =
        await this.certificateService.batchTransferWithTxHash(
          events.map((event) => ({
            ...event.payload,
      ...
      return {
        failedCertificateIds: events.map((e) => e.internalCertificateId),
      };
    }
  }
}
