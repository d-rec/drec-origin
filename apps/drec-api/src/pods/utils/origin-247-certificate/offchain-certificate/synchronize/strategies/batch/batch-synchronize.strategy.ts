  ...
  async synchronize(events: ICertificateEvent[]): Promise<void> {
    ...
    while (!eventsBatchQueue.isEmpty()) {
      ...
      const issueResult = await this.issuancePersistHandler.handleBatch(
        issueEvents.filter(
          (e) => !hasCertificateFailed(e.internalCertificateId),
        ),
      );
      failedCertificateIds.push(...issueResult.failedCertificateIds);
      eventsBatchQueue.removeEventsForCertificateIds(
        issueResult.failedCertificateIds,
      );

      // Batch transfer certificates
      const { events: transferEventsWithBlockchainId, idMap: transferIdMap } =
      ...
      eventsBatchQueue.removeEventsForCertificateIds(
        transferResult.failedCertificateIds,
      );

      // Batch claim certificates
      const { events: claimEventsWithBlockchainId, idMap: claimIdMap } =
        await this.getEventsWithBlockchainId(
          claimEvents.filter(
            (e) => !hasCertificateFailed(e.internalCertificateId),
          ),
      ...
      eventsBatchQueue.removeEventsForCertificateIds(
        claimResult.failedCertificateIds,
      );
    }
  }
