  ...
  implements CertificateReadModelRepository<T>
{
  ...
  async getAll(
    ...
    const generationEndFrom = dateToSeconds(
      options.generationEndFrom ?? new Date(0),
    );
    const generationEndTo = dateToSeconds(
      options.generationEndTo ?? futureDate,
    );
    const generationStartFrom = dateToSeconds(
      options.generationStartFrom ?? new Date(0),
    );
    const generationStartTo = dateToSeconds(
      options.generationStartTo ?? futureDate,
    );
    const creationTimeFrom = dateToSeconds(
      options.creationTimeFrom ?? new Date(0),
    );
