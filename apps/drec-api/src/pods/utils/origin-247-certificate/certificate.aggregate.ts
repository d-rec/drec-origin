...
export class CertificateAggregate<T> {
  ...
  private constructor(events: ICertificateEvent[]) {
    ...
    const initialCertificate = this.createCertificate(
      firstEvent as CertificateIssuedEvent<T>,
    );
  ...
  private apply(certificate: Cert<T>, event: ICertificateEvent): Cert<T> {
    switch (event.type) {
      ...
      case CertificateEventType.ClaimPersisted:
        return this.claimPersisted(
          certificate,
          event as CertificateClaimPersistedEvent,
        );
  ...
  public static fromEvents<T>(
    events: ICertificateEvent[],
  ): CertificateAggregate<T> {
  ...
  private persistError(certificate: Cert<T>): Cert<T> {
    return certificate;
  }
  ...
  private transfer(
    certificate: Cert<T>,
    event: CertificateTransferredEvent,
  ): Cert<T> {
  ...
  private claim(certificate: Cert<T>, event: CertificateClaimedEvent): Cert<T> {
    ...
    const claimedBalance = BigNumber.from(
      certificate.claimers![forAddress]
        ? certificate.claimers![forAddress]
        : '0',
    );
  ...
  private claimPersisted(
    certificate: Cert<T>,
    event: CertificateClaimPersistedEvent,
  ): Cert<T> {
    return {
      ...certificate,
      transactions: [
