...
export class OauthClientCredentialsService {
    ...
    async store(client_id: string, userid: string): Promise<OauthClientCredentials> {
    ...
    async get(api_user_id: string) {
        return await this.clientCredentialsRepository.findOne({api_user_id: api_user_id});
    }



    async replace(id: number, client_id: string, userid: number): Promise<OauthClientCredentials> {
