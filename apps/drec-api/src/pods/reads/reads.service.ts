...
import { toTimezoneDate, toTimezoneDateFormat } from '../../transformers/timezone';
import { validateTimezone } from '../../validations/timezone';
import { Queues } from '../../utils/enums/queues.enum';
import { computeMaxEnergyCapacity } from '../../lib/meter-read';
import { DEFAULT_YIELD_VALUE, DEVICE_DEGRADATION, INFLUX_DB_TIMEOUT } from '../../constants';
...
export class ReadsService {
  ...
  public async findCumulativeValue(device: DeviceDTO) {
    const cumulativeValue = await this.repository.createQueryBuilder('read')
    ...
    return {
      ...
      datetime: cumulativeValue.maxEndDate ? new Date(cumulativeValue.maxEndDate) : new Date(device.createdAt),
    };
  }
  ...
  private async filterMeasurements(
    ...
    if (measurement.type === 'History') {
      return this.processHistoricalReads(device, measurement, lastRead);
  ...
  private async processHistoricalReads(device: DeviceDTO, measurement: NewIntermediateMeterReadDTO, lastRead: ReadDTO) {
  ...
  private async processDeltaReads(device: DeviceDTO, measurement: NewIntermediateMeterReadDTO, lastRead: ReadDTO) {
    if (!lastRead) {
      await new Promise((resolve, reject) => {
        measurement.reads.forEach(async (element, measurementReadIndex) => {
          if (lastRead && lastRead['timestamp']) {
            if (
              ...
              new Date(lastRead.timestamp).getTime()
            ) {
              this.storeFailedReads(
                device.externalId,
                element.value,
              ...
              return reject(
                new ConflictException({
                  success: false,
                  message: `The sent date for reading ${element.endtimestamp} is less than last sent meter read date ${lastRead.timestamp}`,
                }),
              );
            }
          }
        ...
        await new Promise((resolve, reject) => {
          measurement.reads.forEach((element, measurementReadIndex) => {
            this.logger.verbose(`endtimestamp: ${element.endtimestamp}
              ...
              ${typeof lastRead.timestamp}
              timestamp: ${lastRead.timestamp.toISOString()}
              ${typeof lastRead.timestamp.toISOString()}`);
            if (lastRead && lastRead['timestamp']) {
              if (
                new Date(element.endtimestamp).getTime() <
                new Date(lastRead.timestamp).getTime()
                ...
                return reject(
                  new ConflictException({
                    success: false,
                    message: `The sent date for reading ${element.endtimestamp} is less than last sent meter read date ${lastRead.timestamp.toISOString()}`,
                  }),
                );
              }
            }
            ...
            const deltaValidation = this.validateEnergy(read, lastRead, device);
  ...
  private async processAggregateReads(device: DeviceDTO, measurement: NewIntermediateMeterReadDTO, lastRead: ReadDTO) {
  ...
  async findLatestRead(
    ...
    return await this.repository.findOne({
      where: {
        externalId: meterId,
        type: [ReadType.Delta, ReadType.Aggregate],
        order: {
          endDate: 'DESC',
        },
      },
  ...
  private getExistingHistoryDeviceLogFilteredQuery(
    ...
    return this.repository
      ...
      .andWhere(
        new Brackets((db) => {
          db.where('read.start_date BETWEEN :startDate AND :endDate', {
            startDate,
            endDate,
          })
            .orWhere('read.end_date BETWEEN :startDate AND :endDate', {
              startDate,
              endDate,
            })
            .orWhere(':startDate BETWEEN read.start_date AND read.end_date', {
              startDate,
            })
            .orWhere(':endDate BETWEEN read.start_date AND read.end_date', {
              endDate,
            });
        }),
      );
        ...
        new Brackets((db) => {
          db.where(
            new Brackets((db1) => {
              db1
                .where(
                  'read.start_date BETWEEN :reservationStartDate  AND :reservationEndDate',
                  {
            ...
            new Brackets((db2) => {
              db2
                .where(
                  'read.end_date  BETWEEN :reservationStartDate  AND :reservationEndDate',
                  {
                    reservationStartDate: startDate,
                    reservationEndDate: endDate,
                  },
                )
                .orWhere('read.end_date = :reservationEndDate ', {
                  reservationEndDate: endDate,
                });
            }),
          );
  ...
  async getAllRead(
    ...
    if (new Date(deviceOnboarded).getTime() < new Date(filter.end).getTime()) {
      ...
      if (
        ...
        const lastReadOngoingRead = await this.getPaginatedData(
          externalId,
          readsFilter,
          pageNumber,
        );
  ...
  async latestRead(deviceExternalId: string): Promise<any> {
    return this.repository
      ...
      .where('reads.external_id = :meterId', { deviceExternalId })
