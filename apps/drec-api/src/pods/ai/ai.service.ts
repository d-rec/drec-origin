...
export class AiService {
  ...
  async extractMeterIds(
    ...
    const promptInstructions = [
      ...
      ``,
      `WHERE TO LOOK FIRST:`,
      `  - Monitoring portals almost always render a device table with one row per inverter; the SN column is typically labelled "SN", "Serial", "Serial No.", "Inverter SN", "Device SN", "Measurement ID" or appears as a clickable cell next to the inverter model.`,
      `  - SemsPortal: look for the "Inverter" or "Device" tab; the SN sits in the first or second column of the per-device table.`,
      `  - SolarEdge / FusionSolar / PowerTrust: same pattern — a device list with one alphanumeric SN per row.`,
      `  - On a nameplate photo, the SN is the long contiguous string under "SN" / "Serial No.".`,
      `  - SCAN THE WHOLE IMAGE — do not stop at the URL/header. The URL is *not* where SNs live.`,
      ``,
      `WHAT A VALID SN LOOKS LIKE:`,
      `  - Contiguous alphanumeric, typically 10-20 characters (e.g. 9125KGTU24BL0027, 72000WLA24B40034, ES2340051281, 50000HSU012345)`,
      `  - No dashes, no spaces, no unit suffixes`,
      `  - Often starts with a vendor / country prefix but NOT required`,
      `  - When you see a column of similar-looking strings (same length, same letter/digit pattern, one per row), that's almost certainly the SN column — extract every value`,
      ``,
      ...
      `  - Strings in the URL bar / browser address — they are session/request identifiers, never hardware SNs`,
      ...
      ``,
      `If you can find a device table or list, extract every SN you can read from it — multiple SNs per screenshot is the norm, not the exception. Only return an empty array if the image truly contains no device list and no nameplate.`,
      ``,
      `Extract:`,
  ...
  async extractCodFields(
    ...
    const promptInstructions = [
      ...
      ``,
      siteFilterBlock,
      `## ABSOLUTE RULE — units check first, before any field`,
      ``,
      `\`acCapacityKw\` is **AC nameplate, in kW**.`,
      ``,
      `If the doc reports a capacity labelled with any of these markers, the value is the DC side, NOT what acCapacityKw wants:`,
      `  - "kWp" / "Wp" / "MWp" / "kWdc" / "Wdc" / "MWdc"`,
      `  - "PV Capacity (...)" with a kWp / Wp unit`,
      `  - "Peak", "Module rating", "DC capacity", "Array capacity"`,
      ``,
      `In that case: \`acCapacityKw.value = null\`, \`acCapacityKw.confidence = 0\`, \`acCapacityKw.reasoning = "doc reports DC kWp/Wp (peak / module rating), not AC kW"\`.`,
      ``,
      `Example — Provisional Acceptance Certificate column header is "PV Capacity (kWp)" with value 100:`,
      `  acCapacityKw: { value: null, confidence: 0, reasoning: "PV Capacity column is labelled kWp (DC peak), not AC kW" }`,
      ``,
      `kVA and MW (without "p" or "dc") convert normally to kW.`,
      ``,
