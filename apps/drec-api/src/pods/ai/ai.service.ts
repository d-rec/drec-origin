...
export class AiService {
  ...
  async extractSldFields(
    ...
    const prompt = [
      ...
      ``,
      `    Worked example (Atsawa-shape mini-grid): SLD shows 2× HUAWEI PV inverters → AC bus → battery storage (SOLARMD) + Victron Quattro battery inverter + diesel backup (Mikano), with multiple kWh meters and a label like "Nigeria MiniGrid" or "off-grid community". Even though the diagram is "grid-tied" (gridTied=true) and has multiple meters, this is a MINI-GRID feeding local customers — gridExportType MUST be "No (zero-export)". The meters are customer-side (per-household billing) and the "grid" connection if any is for backup IMPORT, not surplus EXPORT to a utility.`,
