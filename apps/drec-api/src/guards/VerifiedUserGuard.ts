...
export class VerifiedUserGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user) {
      this.logger.verbose('No user found in request, skipping verification check');
      return true;
    }

    if (!user.emailVerifiedAt) {
      this.logger.warn(`User ${user.email} attempted to access protected route without email verification`);
