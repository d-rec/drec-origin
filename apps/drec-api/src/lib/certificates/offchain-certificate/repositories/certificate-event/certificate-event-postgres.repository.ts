  ...
  implements CertificateEventRepository
{
  ...
  constructor(
    ...
    this.maxSynchronizationAttemptsForEvent =
      getConfiguration().MAX_SYNCHRONIZATION_ATTEMPTS_FOR_EVENT;
  }
  ...
  public async updateAttempt({
    ...
    const synchronizationAttempts =
      await this.synchronizationAttemptRepository.findByIds(eventIds);
  ...
  public async findAllToProcess(
    params: IGetToProcessOptions,
  ): Promise<CertificateEventEntity[]> {
    ...
    const query = this.repository
      ...
      .andWhere(
        `event.internalCertificateId NOT IN (${failedCertificatesIds.getQuery()})`,
      )
