...
export const useFilterTableData = <Id>(data: TableRowData<Id>[], tableFilters: TableFilter[]) => {
    const [filters, setFilters] = useState<TableFilter[]>(tableFilters);

    const filterTableData = (tableData: TableRowData<Id>[], filtersToApply: TableFilter[]) => {
        const newData =
